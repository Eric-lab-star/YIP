...
const slides: Slide[] = [
  { title: "", bg: "from-blue-50 to-indigo-50", script: "안녕하세요, 여러분. 오늘부터 매우 흥미로운 프로젝트를 시작합니다. 텔레그램에 수학 문제 사진을 올리면 봇이 풀이를 답장해주는 앱을 만드는 것입니다. 한 번에 다 만들 수는 없으므로, 오늘 1부에서는 먼저 텍스트 메시지에 답하는 기본 봇부터 차근차근 만들어보겠습니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><h1 className="text-5xl sm:text-6xl font-bold text-gray-800 leading-tight">텔레그램 봇과 첫 인사</h1><p className="text-2xl text-gray-500 mt-2">수학 봇 만들기 1부 — 기본 봇 만들기</p></div>) },
  { title: "오늘의 학습 목표", bg: "from-yellow-50 to-amber-50", script: "오늘의 학습 목표입니다. 첫째, 챗봇 플랫폼(텔레그램)과 내 코드가 어떻게 연결되는지 이해합니다. 둘째, 텔레그램 봇을 만들고 메시지를 주고받는 기본 동작을 설명할 수 있습니다. 셋째, 봇 토큰이 무엇이고 왜 소중히 다뤄야 하는지 알아봅니다. 넷째, 토큰을 코드에 적지 않고 .env 파일에 따로 보관하는 이유를 설명할 수 있게 됩니다.", content: (<div className="flex flex-col gap-6">{[{ num: "1", text: "챗봇 플랫폼(텔레그램)과 내 코드가 어떻게 연결되는지 이해한다" }, { num: "2", text: "텔레그램 봇을 만들고 메시지를 주고받는 기본 동작을 설명할 수 있다" }, { num: "3", text: "봇 토큰이 무엇이고 왜 소중히 다뤄야 하는지 안다" }, { num: "4", text: "토큰을 코드에 적지 않고 .env에 따로 보관하는 이유를 설명할 수 있다" }].map((item) => (<div key={item.num} className="bg-white/70 rounded-xl p-5 flex items-start gap-4"><span className="bg-blue-500 text-white rounded-full w-9 h-9 flex items-center justify-center shrink-0 font-bold">{item.num}</span><p className="text-xl text-gray-700">{item.text}</p></div>))}</div>) },
  ...
  { title: "봇 토큰 = 우체통의 열쇠", bg: "from-red-50 to-orange-50", script: "봇을 만들려면 텔레그램의 BotFather에게 봇 생성을 요청합니다. BotFather가 토큰이라는 긴 비밀번호를 발급해줍니다. 이 토큰은 '이 우체통은 내 것이에요'라고 증명하는 열쇠입니다. 토큰이 있어야 내 코드가 우체국에서 내 봇의 메시지를 받아올 수 있습니다. 매우 중요한 점은, 토큰은 절대 남에게 보여주면 안 된다는 것입니다. API 키와 동일한 보안 정보이므로, 코드를 공유하거나 인터넷에 올릴 때 토큰이 노출되지 않도록 주의해야 합니다. 그래서 우리는 토큰을 코드에 직접 적지 않고, .env라는 별도 파일에 따로 보관할 것입니다. 집에 비유하면 코드는 도면이고 .env는 현관 열쇠입니다. 도면은 보여줘도 되지만 열쇠는 절대 주지 않습니다. 코드에는 '서랍에서 열쇠를 꺼내와'라는 뜻의 os.environ 한 줄만 적으면 됩니다.", content: (<div className="flex flex-col gap-5"><div className="bg-white/60 rounded-xl p-5"><p className="text-xl text-gray-700"><strong>봇 토큰</strong> = BotFather가 발급하는 긴 비밀번호 = <strong>내 우체통의 열쇠</strong></p></div><div className="bg-red-50 rounded-xl p-5 border-l-4 border-red-400"><p className="text-lg text-gray-700"><strong>토큰은 절대 남에게 보여주면 안 됩니다!</strong></p><p className="text-base text-gray-600 mt-1">API 키와 동일한 보안 정보. 노출 시 /revoke로 토큰 재발급 가능.</p></div><div className="bg-green-50 rounded-xl p-5 border-l-4 border-green-400"><p className="text-lg text-gray-700"><strong>그래서 열쇠는 코드 밖에 둡니다 → <code>.env</code></strong></p><p className="text-base text-gray-600 mt-1">코드 = 집 도면(공유 OK) | <code>.env</code> = 현관 열쇠(공유 절대 금지)</p><p className="text-base text-gray-600 mt-1">코드에는 <code>os.environ[&quot;TELEGRAM_TOKEN&quot;]</code> 한 줄만 적습니다.</p></div></div>) },
  ...
  { title: "메아리 봇 코드 구조", bg: "from-indigo-50 to-violet-50", script: "메아리 봇의 기본 코드 구조를 살펴보겠습니다. 맨 위의 load_dotenv와 os.environ은 .env 파일에서 토큰을 꺼내오는 부분입니다. 코드 어디에도 토큰이 직접 적혀 있지 않다는 점에 주목해주시기 바랍니다. start 함수는 사용자가 /start를 누르면 인사말을 답장합니다. echo 함수는 사용자가 보낸 글자를 그대로 답장합니다. 두 함수 모두 update.effective_message로 메시지를 먼저 꺼낸 뒤, 비어 있을 수 있으므로 if message is None으로 막아주고 사용합니다. add_handler는 '이런 메시지가 오면 이 함수를 써라'라고 봇에게 알려주는 부분입니다. filters.TEXT 앤드 물결표 filters.COMMAND는 '글자이면서 명령어는 아닌 것'만 echo로 보낸다는 뜻입니다. run_polling은 봇을 계속 켜둔 상태로 만들어 메시지를 기다리게 합니다.", content: (<div className="flex flex-col gap-5"><CodeBlock>{`import os
...
async def start(update, context):
    ...
    if message is None:
        return
    await message.reply_text("안녕하세요!")
...
app.run_polling()`}</CodeBlock></div>) },
  { title: "오늘 배운 내용 정리", bg: "from-orange-50 to-red-50", script: "오늘 배운 내용을 정리하겠습니다. 텔레그램 봇은 우체국(서버)을 통해 사용자와 메시지를 주고받습니다. 봇 토큰은 우체통의 열쇠이며, 절대 노출하면 안 됩니다. 그래서 토큰은 코드가 아니라 .env 파일에 보관합니다. 메아리 봇으로 '메시지 받기 → 답장 보내기' 기본 흐름을 익혔습니다. 이 기본 흐름에 AI를 연결하면 똑똑한 봇이 완성됩니다.", content: (<div className="flex flex-col gap-4"><div className="bg-blue-50 rounded-xl p-4"><p className="text-lg text-gray-700">텔레그램 봇 = 우체국(서버)을 통한 메시지 교환</p></div><div className="bg-red-50 rounded-xl p-4"><p className="text-lg text-gray-700">봇 토큰 = 우체통의 열쇠 (절대 노출 금지)</p></div><div className="bg-emerald-50 rounded-xl p-4"><p className="text-lg text-gray-700">열쇠는 코드 밖 <code>.env</code>에 → 코드는 마음껏 공유 가능</p></div><div className="bg-green-50 rounded-xl p-4"><p className="text-lg text-gray-700">메아리 봇으로 기본 흐름 습득: 받기 → 답장</p></div><div className="bg-purple-50 rounded-xl p-4"><p className="text-lg text-gray-700">답장 부분을 AI로 교체하면 → 똑똑한 봇 완성</p></div></div>) },
  { title: "", bg: "from-blue-50 to-indigo-50", script: "오늘 강의를 마치겠습니다. 사용자와 봇 사이의 대화 통로를 이해했습니다. 다음 시간에는 이 봇에 사진을 보는 눈을 달아서 수학 문제 사진을 인식하게 만들겠습니다. 수고하셨습니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><h1 className="text-5xl font-bold text-gray-800">개념 강의를 마칩니다</h1><p className="text-xl text-gray-600 mt-4">다음 시간: 봇에 사진을 보는 눈 달기</p><p className="text-2xl text-gray-500 mt-4">수고하셨습니다!</p></div>) },
];
