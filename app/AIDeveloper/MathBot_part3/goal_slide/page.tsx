...
const slides: Slide[] = [
  ...
  },
  {
    ...
    script: `오늘의 학습 목표는 두 가지입니다. 첫째, 텔레그램 봇이 사진을 받는 방법을 이해합니다. 글자 메시지와 사진 메시지는 처리 방식이 다르며, 이를 위한 별도의 핸들러가 필요합니다. 둘째, 13차시에서 만든 봇 코드와 14차시에서 만든 사진 인식 및 풀이 코드를 결합하여, 사진을 보내면 풀이를 답장하는 완성형 앱을 만듭니다.`,
    content: (
      <div className="flex flex-col gap-6">
        ...
        <div className="space-y-4">
          {[
            ...
            { num: "2", text: "13차시(봇)와 14차시(사진 인식 + 풀이)를 결합해 완성형 앱을 만든다" },
          ].map((item) => (
  ...
  },
  {
    ...
    script: `본격적으로 시작하기 전에, 지금까지 만든 부품을 정리하겠습니다. 13차시에서 만든 것은 텔레그램에서 텍스트 메시지를 받아 답장하는 봇입니다. 이것은 봇의 '입과 귀'에 해당합니다. 14차시에서 만든 것은 사진 속 수학 문제를 인식하고 AI가 단계별 풀이를 생성하는 기능입니다. 이것은 봇의 '눈과 머리'에 해당합니다. 오늘은 이 두 부품을 하나의 몸체에 조립하여, 사진을 받으면 풀이를 답장하는 완성형 봇을 만듭니다. 마치 로봇의 부품을 따로 만들어서 마지막에 조립하는 것과 같습니다.`,
    content: (
  ...
  },
  {
    ...
    script: `사진 한 장이 풀이 결과로 바뀌는 전체 과정을 살펴보겠습니다. 네 단계로 이루어집니다. 첫째, 사용자가 수학 문제 사진을 봇에게 보냅니다. 둘째, 봇이 받은 사진을 컴퓨터에 파일로 저장합니다. 셋째, 저장한 사진을 14차시에서 만든 풀이 기능, 즉 AI에게 전달합니다. 넷째, AI가 만든 풀이 결과를 사용자에게 답장으로 보냅니다. 여기서 중요한 UX 배려 포인트가 있습니다. AI가 풀이를 만드는 데 시간이 걸리므로, 그동안 사용자가 불안해하지 않도록 '잠시만 기다려주세요'와 같은 안내 메시지를 먼저 보내주는 것이 좋습니다.`,
    content: (
      <div className="flex flex-col gap-5">
...
<strong>UX 배려:</strong> 풀이 생성에 시간이 걸리므로, &ldquo;잠시만 기다려주세요...&rdquo; 안내 메시지를 먼저 보내면 친절한 봇이 됩니다.
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="space-y-4">
          {[
            ...
            { num: "3", title: "재사용이 가능하다", desc: "14차시 풀이 기능을 오늘 그대로 재사용하는 것처럼 활용" },
          ].map((item) => (
  ...
  },
  {
    ...
    script: `오늘 봇의 AI에게 줄 명령서, 즉 풀이 프롬프트는 14차시에서 만든 것을 그대로 가져다 씁니다. 새로 만들 필요 없이 부품을 재사용하는 것입니다. 이 프롬프트는 AI에게 '어떻게 풀이를 만들어야 하는지' 알려주는 안내문입니다. 친절한 수학 선생님 역할을 지정하고, 문제 설명, 단계별 풀이 과정, 정답 순서로 답변하도록 지시합니다. 한 번 잘 만들어두면, 사진만 바꿔도 같은 형식의 풀이를 만들어줍니다.`,
    content: (
      <div className="flex flex-col gap-5">
        <div className="bg-white/60 rounded-xl p-4">
          <p className="text-lg text-gray-600">14차시에서 만든 풀이 프롬프트를 <strong>그대로 재사용</strong>합니다.</p>
        </div>
        <CodeBlock>
          {`# 14차시에서 만든 풀이 프롬프트 재사용
solve_prompt = """
너는 친절한 수학 선생님이야.
이 사진 속 수학 문제를 풀어줘.
...
photo_file = await update.message.photo[-1].get_file()`}</CodeBlock>
  ...
  },
  {
    ...
    script: `오늘 강의에서 다룬 내용을 정리하겠습니다. 첫째, 완성형 봇은 13차시의 대화 기능과 14차시의 사진 풀이 기능을 하나로 합쳐서 만듭니다. 둘째, 텔레그램에서 글자와 사진은 서로 다른 핸들러로 처리하며, 사진용으로는 filters.PHOTO를 사용합니다. 셋째, 사진 처리 흐름은 사진 받기, 파일 저장, AI 풀이, 답장 전송 네 단계입니다. 넷째, 사용자 경험을 위해 처리 중 안내 메시지를 먼저 보내는 것이 중요합니다. 다섯째, 이처럼 기능을 따로 만들고 마지막에 합치는 방식을 모듈식 개발이라 하며, 테스트, 디버깅, 재사용에 유리합니다.`,
    content: (
      <div className="flex flex-col gap-4">
        <div className="space-y-3">
          {[
            "13차시(대화) + 14차시(사진 풀이) = 완성형 봇",
