...
const slides: Slide[] = [
  { title: "", bg: "from-purple-50 to-indigo-50", script: "안녕하세요, 여러분. 오늘은 벡터DB와 제미나이를 하나로 합쳐 RAG 챗봇을 직접 만들어보는 실습을 진행하겠습니다. RAG 함수를 완성하고, 다양한 질문으로 테스트한 뒤, Streamlit으로 챗봇까지 만들어보겠습니다. 총 3가지 미션을 약 25~30분에 걸쳐 수행합니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><span className="text-8xl">🔗</span><h1 className="text-5xl sm:text-6xl font-bold text-gray-800">RAG 챗봇 실습 미션</h1><p className="text-2xl text-gray-500 mt-2">나만의 AI 사서 챗봇 만들기</p><p className="text-lg text-gray-400">총 소요 시간: 약 25~30분</p></div>) },
  { title: "실습 전 준비 사항", bg: "from-yellow-50 to-amber-50", script: "준비물을 확인하겠습니다. chromadb, google-genai, streamlit 패키지가 설치되어 있어야 합니다. 지난 시간에 만든 Chroma DB 데이터 또는 청크 데이터가 필요합니다. 없으면 다시 add 해주시면 됩니다. 비교 테스트용 질문 목록도 미리 준비해주시기 바랍니다.", content: (<div className="flex flex-col gap-6"><p className="text-xl text-gray-700">미션 시작 전 아래 사항을 확인합니다.</p><div className="space-y-4">{[{ icon: "📦", text: "chromadb, google-genai, streamlit 패키지 설치" }, { icon: "📄", text: "지난 시간 Chroma DB 데이터 또는 청크 데이터" }, { icon: "🔑", text: "제미나이 API 키 확인" }, { icon: "❓", text: "비교 테스트용 질문 목록 준비" }].map((item, i) => (<div key={i} className="bg-white/70 rounded-xl p-5 flex items-center gap-4"><span className="text-3xl">{item.icon}</span><p className="text-xl text-gray-700">{item.text}</p></div>))}</div></div>) },
  { title: "미션 1: RAG 함수 만들기 (8~10분)", bg: "from-rose-50 to-orange-50", script: "첫 번째 미션입니다. 검색과 생성을 한 함수 안에서 합쳐봅니다. 빈칸이 두 군데 있습니다. 첫째, prompt 안의 역할 빈칸입니다. 어떤 도우미인지 적으면 됩니다. 예를 들어 '친절한 도서관 사서'와 같이 작성합니다. 둘째, question 변수에 테스트할 질문을 문자열로 넣으면 됩니다. 특히 prompt에 '참고 자료에 없는 내용이면 자료에서 찾을 수 없습니다라고 답해줘'라는 조건을 넣어야 합니다. 이것이 AI의 환각을 방지하는 핵심 장치입니다. 8~10분 드리겠습니다.", content: (<div className="flex flex-col gap-5"><div className="bg-white/60 rounded-xl p-4"><p className="text-lg text-gray-600"><strong>목표:</strong> 검색(벡터DB) + 생성(제미나이)을 합친 RAG 함수 완성</p></div><CodeBlock>{`def rag_answer(question):
...
if st.button("질문하기"):
  ...
  { title: "미션 3 해설", bg: "from-teal-50 to-emerald-50", script: "미션 3의 핵심 포인트입니다. Streamlit으로 UI를 만들면 비개발자도 쉽게 사용할 수 있는 앱이 됩니다. 참고한 자료 보기 기능은 RAG의 투명성을 보여주는 중요한 요소입니다. 사용자가 AI의 답변 근거를 직접 확인할 수 있기 때문입니다. 오늘 만든 것은 작은 시작이지만, 더 많은 자료와 기능을 추가하면 실제로 유용한 앱으로 발전시킬 수 있습니다.", content: (<div className="flex flex-col gap-6"><p className="text-2xl text-gray-800 font-semibold">핵심 포인트</p><div className="space-y-4"><div className="bg-white/70 rounded-xl p-5"><p className="text-xl text-gray-700">Streamlit UI = 비개발자도 쉽게 사용 가능</p></div><div className="bg-blue-50 rounded-xl p-5"><p className="text-lg text-gray-700"><strong>&ldquo;참고한 자료 보기&rdquo;</strong> = RAG의 투명성. 답변 근거를 사용자가 직접 확인 가능.</p></div></div></div>) },
  { title: "오늘의 실습 정리", bg: "from-orange-50 to-red-50", script: "오늘 3가지 미션을 모두 수행하셨습니다. 검색과 생성을 합친 RAG 함수 완성, 다양한 질문으로 테스트하며 RAG의 가치 체감, Streamlit으로 나만의 RAG 챗봇 완성까지 해보았습니다. 처음엔 빈 노트에서 시작했던 우리가, 이제는 질문하면 자료를 찾아 친절하게 설명해주는 나만의 AI 비서를 완성한 것입니다.", content: (<div className="flex flex-col gap-5"><div className="space-y-3">{[{ num: "1", text: "검색 + 생성 합친 RAG 함수 완성", color: "bg-rose-100" }, { num: "2", text: "다양한 질문으로 테스트하며 RAG 가치 체감", color: "bg-violet-100" }, { num: "3", text: "Streamlit으로 나만의 RAG 챗봇 완성", color: "bg-teal-100" }].map((item) => (<div key={item.num} className={`${item.color} rounded-xl p-4 flex items-center gap-4`}><span className="text-lg font-bold text-gray-500">미션 {item.num}</span><p className="text-lg text-gray-700">{item.text} ✅</p></div>))}</div></div>) },
