...
const slides: Slide[] = [
  { title: "", bg: "from-purple-50 to-indigo-50", script: "안녕하세요, 여러분. 오늘은 벡터DB와 제미나이를 하나로 합쳐 RAG 챗봇을 직접 만들어보는 실습을 진행하겠습니다. 먼저 실제 문서 파일을 통째로 읽어 벡터DB에 넣는 인덱싱부터 시작해서, RAG 함수를 완성하고, 다양한 질문으로 테스트한 뒤, Streamlit으로 챗봇까지 만들어보겠습니다. 총 4가지 미션을 약 35~40분에 걸쳐 수행합니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><h1 className="text-5xl sm:text-6xl font-bold text-gray-800">RAG 챗봇 실습 미션</h1><p className="text-2xl text-gray-500 mt-2">나만의 AI 사서 챗봇 만들기</p><p className="text-lg text-gray-400">총 소요 시간: 약 35~40분</p></div>) },
  { title: "실습 전 준비 사항", bg: "from-yellow-50 to-amber-50", script: "준비물을 확인하겠습니다. chromadb, google-genai, streamlit 패키지가 설치되어 있어야 합니다. 그리고 오늘은 실습용 문서 파일이 필요합니다. 실습 페이지에 있는 예시 안내문을 school_guide.txt라는 이름으로 저장해도 되고, 학교 가정통신문이나 동아리 소개글처럼 직접 가진 문서를 사용해도 좋습니다. 비교 테스트용 질문 목록도 미리 준비해주시기 바랍니다. 미션 1의 보너스로 PDF 인덱싱도 준비해두었습니다. 해보고 싶은 분은 pypdf 패키지와 글자로 된 PDF 파일을 하나 준비하시면 되는데, 선택 사항이니 없어도 실습에는 문제가 없습니다.", content: (<div className="flex flex-col gap-6"><p className="text-xl text-gray-700">미션 시작 전 아래 사항을 확인합니다.</p><div className="space-y-4">{[{ text: "chromadb, google-genai, streamlit 패키지 설치" }, { text: "실습용 문서 파일(.txt) — 예시 안내문 또는 내가 가진 문서" }, { text: "제미나이 API 키 확인" }, { text: "비교 테스트용 질문 목록 준비" }, { text: "(선택) PDF로도 해보려면 pypdf 패키지 + 글자로 된 PDF 파일" }].map((item, i) => (<div key={i} className="bg-white/70 rounded-xl p-5 flex items-center gap-4"><p className="text-xl text-gray-700">{item.text}</p></div>))}</div></div>) },
  { title: "미션 1: 실제 문서를 벡터DB에 넣기 (8~10분)", bg: "from-lime-50 to-green-50", script: "첫 번째 미션입니다. 지금까지는 코드에 직접 적어둔 짧은 예시 문장으로 연습했지만, 오늘은 진짜 문서 파일을 통째로 읽어서 벡터DB에 넣어봅니다. 먼저 실습 페이지의 예시 안내문을 복사해서 school_guide.txt라는 이름으로, 코드 파일과 같은 폴더에 저장합니다. 직접 가진 문서를 사용해도 좋습니다. 코드 맨 위의 GeminiEmbedding 클래스는 Chroma에게 임베딩을 제미나이에게 맡기라고 알려주는 연결 코드입니다. Chroma의 기본 임베딩 모델은 영어 위주로 학습되어 한국어 검색이 부정확하기 때문입니다. 이 부분은 그대로 복사해서 쓰면 됩니다. 빈칸은 두 군데입니다. 첫째, open 함수에는 방금 저장한 파일 이름을 문자열로 넣습니다. 둘째, split 앞에는 파일에서 읽어온 텍스트가 담긴 변수를 넣습니다. 청킹은 빈 줄, 즉 문단 기준으로 자릅니다. 200자씩 기계적으로 자르면 문장이 중간에 끊겨서 검색이 잘 안 되기 때문입니다. 3차시에서 배운 대로 PersistentClient를 써서 디스크에 저장하고, upsert로 저장하기 때문에 코드를 다시 실행해도 안전합니다. 파일을 찾을 수 없다는 FileNotFoundError가 나면 txt 파일이 코드 파일과 같은 폴더에 있는지 확인하고, 코랩을 쓴다면 왼쪽 폴더 아이콘을 눌러 파일을 업로드하면 됩니다. 8~10분 드리겠습니다.", content: (<div className="flex flex-col gap-5"><div className="bg-white/60 rounded-xl p-4"><p className="text-lg text-gray-600"><strong>목표:</strong> 실제 문서 파일을 읽고 → 청킹하고 → 제미나이 임베딩으로 벡터DB에 저장 (인덱싱)</p></div><CodeBlock>{`# 제미나이 임베딩을 Chroma에 연결 (그대로 복사)
...
# 디스크의 chroma_db 폴더에 저장 (3차시의 my_documents와 헷갈리지
# 않도록 school_guide로 이름 붙인다)
...
print(rag_answer(question))`}</CodeBlock><div className="bg-white/70 rounded-xl p-4"><p className="text-lg text-gray-600">역할 예시: &quot;친절한 도서관 사서&quot;, &quot;학교 안내 도우미&quot;</p></div></div>) },
  ...
  { title: "미션 4: Streamlit RAG 챗봇 (8~10분)", bg: "from-teal-50 to-cyan-50", script: "마지막 미션입니다. rag_answer 함수를 Streamlit 화면에 연결하여 입력창에 질문을 받아 답을 보여주는 진짜 RAG 챗봇을 완성합니다. 여기서 중요한 점이 있습니다. Streamlit은 버튼을 누를 때마다 스크립트를 처음부터 다시 실행하기 때문에, 미션 1의 인덱싱 코드를 그냥 두면 질문할 때마다 문서를 또 임베딩하게 됩니다. 그래서 인덱싱 코드를 get_collection 함수로 감싸고 st.cache_resource를 붙여서, 앱이 켜져 있는 동안 딱 한 번만 실행되게 만듭니다. 빈칸은 rag_answer(user_question) 형태로 함수를 호출하면 됩니다. 터미널에서 streamlit run app.py로 실행하고 다양한 질문을 넣어보시기 바랍니다. 첫 질문은 인덱싱 때문에 조금 걸리지만, 두 번째 질문부터는 훨씬 빨라지는 것을 확인할 수 있습니다. Streamlit 연결이 어려우신 분들은 .ipynb에서 rag_answer 함수만 다양한 질문으로 실행하는 것으로도 충분합니다. 8~10분 드리겠습니다.", content: (<div className="flex flex-col gap-5"><div className="bg-white/60 rounded-xl p-4"><p className="text-lg text-gray-600"><strong>목표:</strong> rag_answer를 Streamlit에 연결하여 RAG 챗봇 완성 (인덱싱은 캐싱으로 한 번만)</p></div><CodeBlock>{`import streamlit as st

st.title("나만의 RAG 챗봇")

# 미션 1의 인덱싱 코드를 함수로 감싸고 @st.cache_resource를 붙였다.
# 화면이 다시 그려져도 이 함수는 앱이 켜져 있는 동안 딱 한 번만 실행된다.
@st.cache_resource
def get_collection():
    # 미션 1의 GeminiEmbedding + PersistentClient + upsert 코드를 그대로 가져온다
    ...
    return collection
...
if st.button("질문하기"):
  ...
  { title: "", bg: "from-purple-50 to-indigo-50", script: "오늘 실습을 마치겠습니다. RAG의 4차시 여정이 완성되었습니다. 임베딩, 청킹, 벡터DB, 그리고 오늘의 문서 인덱싱과 RAG 파이프라인까지, 여러분은 실제 문서를 AI에게 전달하는 검색 증강 생성의 전체 과정을 직접 구현해보셨습니다. 수고하셨습니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><h1 className="text-5xl font-bold text-gray-800">RAG 챗봇 완성!</h1><p className="text-xl text-gray-600 mt-4">임베딩 → 청킹 → 벡터DB → RAG 파이프라인</p><p className="text-2xl text-gray-500 mt-4">수고하셨습니다!</p></div>) },
];
