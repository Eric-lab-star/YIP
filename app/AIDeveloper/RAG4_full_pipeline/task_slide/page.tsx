...
const slides: Slide[] = [
  ...
  { title: "실습 전 준비 사항", bg: "from-yellow-50 to-amber-50", script: "준비물을 확인하겠습니다. chromadb, google-genai, streamlit 패키지가 설치되어 있어야 합니다. 그리고 오늘은 실습용 문서 파일이 필요합니다. 실습 페이지에 있는 예시 안내문을 school_guide.txt라는 이름으로 저장해도 되고, 학교 가정통신문이나 동아리 소개글처럼 직접 가진 문서를 사용해도 좋습니다. 비교 테스트용 질문 목록도 미리 준비해주시기 바랍니다.", content: (<div className="flex flex-col gap-6"><p className="text-xl text-gray-700">미션 시작 전 아래 사항을 확인합니다.</p><div className="space-y-4">{[{ text: "chromadb, google-genai, streamlit 패키지 설치" }, { text: "실습용 문서 파일(.txt) — 예시 안내문 또는 내가 가진 문서" }, { text: "제미나이 API 키 확인" }, { text: "비교 테스트용 질문 목록 준비" }].map((item, i) => (<div key={i} className="bg-white/70 rounded-xl p-5 flex items-center gap-4"><p className="text-xl text-gray-700">{item.text}</p></div>))}</div></div>) },
