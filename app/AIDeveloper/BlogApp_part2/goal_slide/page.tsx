...
const slides: Slide[] = [
  { title: "", bg: "from-purple-50 to-pink-50", script: "안녕하세요, 여러분. 지난 시간에는 주제만 주면 제목부터 본문까지 자동 생성하는 글 생성 기능을 만들었습니다. 오늘은 그 글에 어울리는 대표 이미지까지 AI로 진짜로 그려서, 글과 그림이 한 팀이 되는 완성형 블로그 앱을 만들어보겠습니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><span className="text-8xl">🎨</span><h1 className="text-5xl sm:text-6xl font-bold text-gray-800 leading-tight">블로그 앱 완성하기 (2부)</h1><p className="text-2xl text-gray-500 mt-2">글에 그림까지! 완성형 블로그 앱</p></div>) },
  ...
  { title: "잠깐! 오늘의 예외 상황", bg: "from-red-50 to-orange-50", script: "이미지 생성을 배우기 전에 꼭 알아야 할 것이 있습니다. 지금까지 우리가 쓴 제미나이 API는 모두 무료였습니다. 하지만 이미지를 진짜로 그려주는 기능은 2026년 현재 무료 등급이 없고, 결제, 즉 빌링이 활성화된 계정에서만 사용할 수 있습니다. 우리 반에 결제가 안 된 친구가 있어도 괜찮습니다. 오늘 코드는 이미지 생성이 실패해도 에러 없이 안내 메시지만 보여주도록 만들 것이고, 글 생성 기능은 이미지와 상관없이 그대로 잘 작동합니다.", content: (<div className="flex flex-col gap-6"><div className="bg-white/70 rounded-xl p-6"><p className="text-xl text-gray-800"><strong>지금까지:</strong> 제미나이 API는 전부 <strong>무료</strong>였습니다</p></div><div className="bg-red-50 rounded-xl p-6 border-l-4 border-red-400"><p className="text-xl text-gray-800"><strong>오늘부터:</strong> 이미지 생성은 <strong>결제(billing)가 활성화된 계정</strong>에서만 가능합니다 (2026년 기준)</p></div><div className="bg-green-50 rounded-xl p-4"><p className="text-lg text-gray-700">💡 걱정 마세요! 결제가 안 되어 있어도 앱이 멈추지 않도록 코드를 만들 겁니다. 글 생성 기능은 그대로 작동합니다.</p></div></div>) },
...
print("이미지 묘사:", image_description)`}</CodeBlock></div>) },
  { title: "코드 미리보기: 진짜로 이미지 그리기", bg: "from-violet-50 to-purple-50", script: "이제 이 묘사문으로 진짜 그림을 그려보겠습니다. 이미지를 그리는 모델은 지금까지 쓰던 gemini-3.5-flash와 다릅니다. 이미지 생성 전용 모델인 gemini-3.1-flash-image, 별명 나노 바나나 2를 쓰고, response_modalities라는 설정으로 글자뿐 아니라 그림도 달라고 알려줘야 합니다. 나머지는 똑같이 client.models.generate_content입니다. 응답 안에서 part.as_image()로 그림 조각을 꺼내 save()로 저장하면 끝입니다.", content: (<div className="flex flex-col gap-5"><CodeBlock>{`from google.genai import types
...
for part in image_response.parts:
    if image := part.as_image():
        ...
        print("표지 이미지를 저장했습니다: cover.png")`}</CodeBlock><div className="bg-white/70 rounded-xl p-4"><p className="text-lg text-gray-700">이렇게 만든 <code>cover.png</code>를 <code>st.image()</code>에 넘기면 화면에 진짜 표지 그림이 나타납니다!</p></div></div>) },
  { title: "오늘 배운 내용 정리", bg: "from-orange-50 to-red-50", script: "오늘 배운 내용을 정리하겠습니다. 이미지 묘사 프롬프트도 글 프롬프트와 원리가 동일합니다. 구체적 묘사, 분위기, 스타일을 담아야 합니다. 이미지를 그릴 때는 gemini-3.5-flash가 아닌 gemini-3.1-flash-image 같은 이미지 전용 모델을 쓰고, response_modalities를 설정해야 합니다. 이미지 생성은 결제가 활성화된 계정에서만 가능하다는 점도 기억해야 합니다. 여러 AI 기능을 하나의 앱에 통합하는 멀티 기능 통합의 개념을 배웠습니다.", content: (<div className="flex flex-col gap-4"><div className="bg-green-50 rounded-xl p-4"><p className="text-lg text-gray-700">✅ 이미지 묘사 프롬프트 = 구체적 묘사 + 분위기 + 스타일</p></div><div className="bg-blue-50 rounded-xl p-4"><p className="text-lg text-gray-700">✅ 이미지 생성은 gemini-3.1-flash-image + response_modalities</p></div><div className="bg-red-50 rounded-xl p-4"><p className="text-lg text-gray-700">✅ 이미지 생성은 결제(billing)가 활성화된 계정에서만 가능</p></div><div className="bg-purple-50 rounded-xl p-4"><p className="text-lg text-gray-700">✅ 멀티 기능 통합: 글 생성 + 이미지 생성 = 1인 출판사 앱</p></div></div>) },
  { title: "", bg: "from-purple-50 to-pink-50", script: "오늘 강의를 마치겠습니다. 글만 쓰던 앱이 진짜 그림까지 그리는 1인 출판사로 한 단계 발전했습니다. 이제 실습에서 직접 만들어보겠습니다. 수고하셨습니다.", content: (<div className="flex flex-col items-center justify-center h-full gap-6 text-center"><span className="text-8xl">📘</span><h1 className="text-5xl font-bold text-gray-800">개념 강의를 마칩니다</h1><p className="text-xl text-gray-600 mt-4">다음: 이미지 생성 + Streamlit 통합 실습</p><p className="text-2xl text-gray-500 mt-4">수고하셨습니다! 🐾</p></div>) },
];
