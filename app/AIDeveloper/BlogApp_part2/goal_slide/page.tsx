...
const slides: Slide[] = [
  ...
  { title: "코드 미리보기: 이미지 묘사 생성", bg: "from-indigo-50 to-violet-50", script: "실습에서 사용할 코드를 미리 살펴보겠습니다. topic 변수에 주제를 넣고, AI에게 그 주제의 대표 이미지를 묘사하는 영어 문장을 만들어달라고 요청합니다. 여기서 맨 마지막 줄, '설명이나 인사말 없이 영어 묘사문 한 문장만 출력해'가 오늘의 진짜 핵심입니다. 이 줄이 없으면 AI가 친절하게 한국어 설명을 잔뜩 붙여서 답합니다. 사람이 읽기엔 좋지만, 그 설명 덩어리가 그대로 그림 모델에게 넘어가면 그림 모델이 이걸 그림 주문이 아니라 말 걸기로 알아듣습니다. 그래서 그림 대신 한국어 답장만 돌아옵니다. 에러도 나지 않아서 더 헷갈립니다. 이 줄을 넣으면 warm and cozy home cafe with wooden table 같은 순수한 영어 묘사문만 나오고, 이걸 그대로 그림 모델에게 넘겨줍니다.", content: (<div className="flex flex-col gap-5"><CodeBlock>{`topic = "초보자를 위한 홈카페 만들기"
...
print("이미지 묘사:", image_description)`}</CodeBlock><div className="bg-amber-50 rounded-xl p-4 border-l-4 border-amber-400"><p className="text-lg text-gray-700">마지막 <strong>&quot;중요:&quot;</strong> 줄이 빠지면 AI가 한국어 설명을 붙여서 답하고, 그 덩어리를 받은 그림 모델은 <strong>그림 대신 답장</strong>을 보냅니다!</p></div></div>) },
...
for part in image_response.parts or []:
  ...
  { title: "오늘 배운 내용 정리", bg: "from-orange-50 to-red-50", script: "오늘 배운 내용을 정리하겠습니다. 이미지 묘사 프롬프트도 글 프롬프트와 원리가 동일합니다. 구체적 묘사, 분위기, 스타일을 담아야 합니다. 그리고 오늘 가장 중요한 것은, AI 두 개를 연결할 때 앞 AI의 출력이 곧 뒤 AI의 입력이 되므로, 설명 없이 영어 한 문장만 출력하라고 출력 형식을 반드시 못박아야 한다는 점입니다. 이미지를 그릴 때는 gemini-3.5-flash가 아닌 gemini-3.1-flash-lite-image 같은 이미지 전용 모델을 쓰고, response_modalities를 설정해야 합니다. 이 모델은 일반 API 키로도 작동이 확인되었지만, 계정이나 모델에 따라 결제 여부가 영향을 줄 수 있다는 점도 기억해야 합니다. 여러 AI 기능을 하나의 앱에 통합하는 멀티 기능 통합의 개념을 배웠습니다.", content: (<div className="flex flex-col gap-4"><div className="bg-green-50 rounded-xl p-4"><p className="text-lg text-gray-700">이미지 묘사 프롬프트 = 구체적 묘사 + 분위기 + 스타일</p></div><div className="bg-rose-50 rounded-xl p-4 border-l-4 border-rose-400"><p className="text-lg text-gray-700">AI를 연결할 땐 <strong>출력 형식을 못박기</strong> — &quot;설명 없이 영어 한 문장만&quot;</p></div><div className="bg-blue-50 rounded-xl p-4"><p className="text-lg text-gray-700">이미지 생성은 gemini-3.1-flash-lite-image + response_modalities</p></div><div className="bg-amber-50 rounded-xl p-4"><p className="text-lg text-gray-700">일반 API 키로 작동 확인 (단, 계정/모델별로 결제 여부가 영향 가능)</p></div><div className="bg-purple-50 rounded-xl p-4"><p className="text-lg text-gray-700">멀티 기능 통합: 글 생성 + 이미지 생성 = 1인 출판사 앱</p></div></div>) },
