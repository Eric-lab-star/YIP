...
for part in image_response.parts:
    if image := part.as_image():
        ...
        print("표지 이미지 저장 완료: cover.jpg")`}</CodeBlock><div className="bg-white/70 rounded-xl p-4"><p className="text-lg text-gray-600">💡 빈칸: &quot;따뜻하고 아늑한&quot;, &quot;차분하고 깔끔한&quot; 등</p></div></div>) },
  { title: "미션 1 해설", bg: "from-rose-50 to-pink-50", script: "미션 1의 핵심 포인트입니다. AI에게 이미지 묘사문을 만들게 하면, 주제의 핵심 키워드를 자동으로 추출하여 적절한 영어 묘사를 생성합니다. 그다음 이 묘사문을 이미지 전용 모델에 전달하면 진짜 그림을 얻을 수 있습니다. response.parts를 하나씩 살펴보면서 part.as_image()로 그림 조각을 꺼내 저장하는 것이 핵심입니다. 분위기 단어를 바꾸면 같은 주제라도 전혀 다른 느낌의 이미지가 나옵니다.", content: (<div className="flex flex-col gap-6"><p className="text-2xl text-gray-800 font-semibold">핵심 포인트</p><div className="bg-white/70 rounded-xl p-5"><p className="text-xl text-gray-700">AI가 주제 키워드를 자동 추출 → <strong>적절한 영어 묘사문 생성</strong> → <strong>진짜 이미지로 변환</strong></p></div><div className="bg-blue-50 rounded-xl p-4"><p className="text-lg text-gray-600">part.as_image()로 그림 조각을 꺼내 cover.jpg로 저장</p></div><div className="bg-red-50 rounded-xl p-4"><p className="text-lg text-gray-600">결제가 안 되어 있으면 이미지가 없거나 에러가 날 수 있습니다</p></div></div>) },
