...
#...main.py의 기존 코드
		...
		des: (
			<>
				새로운 그룹을 만들어서 운석들을 관리해봅시다. main.py의 main함수 안에{" "}
				<Code>meteor_sprite_group = pygame.sprite.Group()</Code>를 추가하세요.
				이렇게 하면 나중에 플레이어와 운석이 충돌하는지 쉽게 확인할 수 있습니다.
			</>
		),
	},
	{
...
#...main.py의 기존 코드
		...
		des: (
			<>
				...
				<Code>meteor_sprite_group</Code>와 <Code>all_sprite_group</Code>를 모두
				넣어서 생성합니다.
			</>
		),
	},
...
#... meteor.py의 기존 코드
		...
		des: (
			<>
				그룹을 튜플의 형태로 넣어주기 때문에 클래스에서 <Code>__init__</Code>가
				정의된 부분에서 <Code>group</Code>의 형태를 튜플로 바꿔줘야합니다.
			</>
		),
	},
	{
...
#...main.py의 기존 코드
		...
		des: (
			<>
				...
				<Code>pygame.sprite.spritecollide()</Code> 함수를 사용하면 됩니다. 이
				함수는 첫 번째 인자로 충돌을 확인할 스프라이트(여기서는 플레이어)를
				받고, 두 번째 인자로 충돌을 확인할 스프라이트 그룹(여기서는 운석 그룹)을
				받습니다. 세 번째 인자는 충돌이 감지되었을 때 해당 스프라이트를 그룹에서
				제거할지 여부를 나타내는데, 여기서는 아직 제거하지 않을 것이므로{" "}
				<Code>False</Code>로 설정합니다. 이 코드를 추가한 후 게임을 실행하면,
				플레이어가 운석과 충돌할 때마다 콘솔에 충돌한 운석들의 리스트가 출력되는
				것을 볼 수 있습니다.
			</>
		),
	},
...
const missileCollisionCodeBlock = [
	{
		code: `
# settings.py 코드
import pygame
...
from settings import (
    ...
    missile_sprite_group,  # <-- 추가하기
		...
		des: (
			<>
				settings.py에서 새롭게 만든 그룹들을 main.py로 가져옵니다. 이렇게 하면
				main.py에서 그룹들을 따로 만들 필요가 없어집니다. main.py에서 그룹들을
				제거하는 것도 잊지 마세요.
			</>
		),
	},
...
# ...main.py의 main 함수 while 루프안 코드
		...
		des: (
			<>
				...
				pygame.sprite.groupcollide함수를 사용해야 합니다. 충돌은 매 프레임 마다
				감지해야하기 때문에 while 루프 안에 코드를 추가해야합니다. 이 함수는 첫
				번째 인자로 충돌을 확인할 스프라이트 그룹(여기서는 운석 그룹)을 받고, 두
				번째 인자로 충돌을 확인할 스프라이트 그룹(여기서는 미사일 그룹)을
				받습니다. 세 번째와 네 번째 인자는 충돌이 감지되었을 때 해당
				스프라이트를 그룹에서 제거할지 여부를 나타내는데, 여기서는 충돌이
				감지되면 운석과 미사일 모두 제거되도록 <Code>True</Code>로 설정합니다.
			</>
		),
	},
	{
...
class Meteor(pygame.sprite.Sprite):
    ...
    def update(self, dt: float):
		...
		des: (
			<>
				운석 클래스 또한 인스턴스를 생성할 때 그룹을 전달할 필요가 없게 수정을
				합니다.
			</>
		),
	},
...
#... Player 클래스 기존 코드
		...
		des: (
			<>
				Player 클래스에서는 Missile 인스턴스를 생성하는 부분 또한 있음으로 두
				부분다 수정해야합니다. 또한 파일의 상단에 settings.py에서 새롭게 만든
				그룹들을 가져오는 것도 잊지 마세요.
			</>
		),
	},
...
class Missile(pygame.sprite.Sprite):
    ...
    def update(self, dt: float):
		...
		des: (
			<>
				Missile 클래스 또한 인스턴스를 만들때 그룹을 인수로 받을 필요없게 하기
				위해서 수정을 합니다.
			</>
		),
	},
...
export default function Page() {
	return (
		<div className="p-10 pb-100">
			...
			{spriteCollideCodeBlock.map((v, i) => (
...
#...main.py의 기존 코드
			...
			{codeCheck.map((v, i) => (
				<ToggleCodeBlock key={i} header={v.header} code={v.code} />
			))}
			...
			{missileCollisionCodeBlock.map((v, i) => (
				<CodeBlockExplainSection
					code={v.code}
					title={v.title}
					des={v.des}
