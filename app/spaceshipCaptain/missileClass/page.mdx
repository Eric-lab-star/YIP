...
export const metadata = {
  title: "미사일 클래스",
  description: "미사일 스프라이트 클래스를 만들고, 스페이스키를 누르면 우주선에서 미사일이 발사되도록 쿨다운과 함께 구현해봐요.",
};

# 미사일 클래스

미사일 클래스는 entity/missile.py 파일을 새로 만들어 작성합니다. 미사일 이미지 또한 아래의 버튼을 클릭하고 images 폴더에 저장합니다.
...
이 코드는 게임에 등장할 미사일 객체를 만드는 설계도예요. `Missile.path`에 이미지 파일 경로를 저장해두고, `__init__`에서 `super().__init__(group)`으로 스프라이트 그룹에 등록하면서 `self.image`로 겉모습을, `self.rect`로 화면 위 위치를 지정해요. `update`는 지금은 비어 있지만 나중에 미사일이 움직이는 로직을 채워 넣을 자리예요.
...
### main.py에서 미사일 객체 만들기

main.py 파일에서 미사일 객체를 만들고 프로그램을 실행시켜서 화면에 미사일이 잘 나오는지 확인해보세요.
...
미사일이 화면에 잘 나오는 것을 확인했다면 이제부터는 스페이스키를 눌렀을 때 우주선에서 미사일이 나오도록 프로그래밍을 할 거예요.

```python
#entity/missile.py 수정하기
#...기존 미사일 코드
class Missile(pygame.sprite.Sprite):
    path: str = join("images", "missile.png")
    speed: float = 200 # <-- 수정
...
미사일의 `speed` 속성을 추가해서 미사일이 움직이는 속도를 지정해주고, `__init__`에서는 미사일이 생성될 때 위치를 받아서 그 위치에서 미사일이 나오도록 수정해요. `update`에서는 매 프레임마다 미사일이 위로 움직이도록 좌표를 업데이트하고, 화면 밖으로 나가면 `kill()`로 제거되도록 해요.
...
이제 player.py 파일에 Missile 클래스를 불러와서 player에서 미사일 인스턴스를 생성할 수 있게 만들 거예요.
...
### player.py: 미사일 발사에 필요한 변수 추가하기

미사일 발사에 필요한 변수를 추가했어요. `missile_cooldown`은 미사일을 너무 많이 발사하지 못하게 제한하는 변수이고 `missile_timer`는 최근 미사일 발사 시간을 저장하는 변수예요.
...
### player 클래스 init 메서드 수정하기

미사일에도 group을 전달하기 위해서 main.py 파일에서 draw와 update 메서드를 호출했을 때 미사일이 그려지도록 하기 위해서 group을 변수로 설정해요.
...
`handleMissile` 메서드는 미사일과 관련된 일을 처리하는 함수예요.
...
### handleMissile 메서드 추가하기

이 메서드는 최근 미사일 발사 시간과 현재 시간의 차이를 구해서 미사일 쿨다운 시간보다 크다면 미사일을 발사하는 함수예요.
