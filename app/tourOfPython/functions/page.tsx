...
export default async function Page() {
	...
	return (
		<div className="mb-30">
			<Link id="title" href={"/tourOfPython/functions#title"}>
				<Title my="m">함수 Function</Title>
			</Link>
			<Link id="create" href={"/tourOfPython/functions#create"}>
...
<Code>def</Code>는 영어 define 을 의미해요. 한국어로 정의하다라는 의미에요. 다시 말해 파이썬을 이용해서 “game_over를 정의해” 라고 컴퓨터에게 작성하고 있는 거에요.
			...
			<Link id="simple_create" href={"/tourOfPython/functions#simple_create"}>
...
`
			...
			<Link id="why_create" href={"/tourOfPython/functions#why_create"}>
...
왼쪽에 있는 코드를 실행시켜 볼까요?
			...
			<Link id="function_body" href={"/tourOfPython/functions#function_body"}>
...
함수 바디에 어떤 것도 넣고 싶지 않다면, pass 를 넣어도 좋아요.  pass 가 뭔지 이해가 아직 안 되겠지만 지금은 오류를 무시해주는 마법이라고 생각하세요
			...
			<Link id="parameter" href={"/tourOfPython/functions#parameter"}>
...
또한 <Code>return</Code>은 함수를 끝내는 역할을 하기 때문에 return 뒤오는 코드는 실행되지 않아요.
		...
		<Link id="quizz" href={"/tourOfPython/functions#quizz"}>
...
`
			...
			<NextAndPrev
			prev={"/tourOfPython/variable_string_boolean"}
			prevPage="변수, 문자열, 불리안"
			next={"/tourOfPython/challenge_hello"}
