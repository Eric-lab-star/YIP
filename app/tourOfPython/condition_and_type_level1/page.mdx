...
export const metadata = {
  title: "조건문과 타입 Level 1",
  description: "if/elif/else 와 try...except 코드의 실행 결과를 예측하는 개념 확인 문제를 풀어봐요.",
};

# 조건문과 타입 Level 1

<QuizzWithOptions answer={4} question={`1. 코드 실행 결과 "사망"이 출력되도록 하는 score의 값으로 올바른 것을 고르세요.`}>
  <CodeBlock code={`if score > 50:
    print("생존")
else:
    print("사망")
`} />
...
<QuizzWithOptions answer={4} question="3. 다음 코드에 관한 설명으로 틀린 것을 고르세요.">
  <CodeBlock code={`if time >= 50:
    ...
    print("빨간불")
`} />
  <Option value={1}>if의 조건이 참이 되면 초록불이 출력된다.</Option>
  <Option value={2}>time의 값이 20이라면 노란불이 출력된다.</Option>
  <Option value={3}>time의 값이 0이라면 빨간불이 출력된다.</Option>
...
`} />
  ...
  <Option value={2}>코드를 실행 후 input함수는 입력을 받을 때까지 기다린다.</Option>
...
`} />
  ...
  <Option value={2}>3과목 중 한 개라도 100보다 큰 값을 입력하면 등급이 나오지 않는다.</Option>
  <Option value={3}>3과목 중 두 개 이상 70점이면 C 등급이 된다.</Option>
  <Option value={4}>int함수는 오류를 만들 수 있다.</Option>
</QuizzWithOptions>
